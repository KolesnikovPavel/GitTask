...
namespace GitTask
{
    class Program
    {
        ...
        {
            var commandNames = Regex.Matches("Git, Update, Commit, Update, Checkout", @"\w+").Cast<Match>().Select(x => x.Value).ToArray();
            var commandArgs = Regex.Matches("[3],[0,5],[],[0, 6],[50000, 0]", @"\[([\d, ]*)\]").Cast<Match>()
                .Select(
                    x => x.Groups[1].Value
                        .Split(',')
